...
NumericMatrix ComputeECDF_byAtomRanks(IntegerVector Atomrank_x, IntegerVector Atomrank_y, IntegerVector NR_Atoms){
  ...
  //find atoms membership
  for(int i=0;i<n;i++){
    B(Atomrank_x(i),Atomrank_y(i)) += 1.0;
  }
  for(int s = 1 ; s <= nr_atoms; s++){
    for(int r = 1 ; r <= nr_atoms; r++){
      A(r,s) = B(r,s-1) + B(r-1,s) - B(r-1,s-1) +B(r,s);
...
double compute_expected_uneven_atoms( int xrank_low,int xrank_high, int yrank_low, int yrank_high, int n){
  bool debug = false;
  double res = ((double)(xrank_high - xrank_low)* (yrank_high - yrank_low))/((float)n);
  if(debug){
    Rprintf("#########\n\r");
    Rprintf("DEBUG EXP\n\r");
    Rprintf("xranklow: %d xrank_high: %d yrank_low: %d yrank_high: %d \n\r",
            xrank_low,xrank_high,yrank_low,yrank_high);
    Rprintf("EXP: %lf \n\r",res);
    Rprintf("#########\n\r");
  }
  return res;
...
// [[Rcpp::export]]
List rcpp_Compute_ADP_MAX_3X3_over_uneven_atoms(IntegerVector Atomrank_x,IntegerVector Atomrank_y,
                                         IntegerVector AtomPoint_xaxis, IntegerVector AtomPoint_yaxis){
  bool debug = false;
  //compute ecdf
  int nr_atoms = AtomPoint_xaxis.length()-1; // first entry is zero, last entry is at N
  if(debug){Rprintf("NR ATOMS = %d \n\r",nr_atoms);}
  
  int n = Atomrank_x.length();
  IntegerVector nr_atoms_vector(1); nr_atoms_vector(0)=nr_atoms;
  NumericMatrix ecdf = ComputeECDF_byAtomRanks(Atomrank_x,Atomrank_y,nr_atoms_vector);
  if(debug){
    Rprintf("ECDF: \n\r");
    Rprintf("ECDF nrow: %d \n\r",ecdf.nrow());
    Rprintf("ECDF ncol: %d \n\r",ecdf.ncol());
  ...
  for(int i = 1; i < nr_atoms-1 ; i++){
    for(int j = i+1; j < nr_atoms ; j++){
      for(int i2 = 1; i2 < nr_atoms-1 ; i2++){
        for(int j2 = i2+1; j2 < nr_atoms ; j2++){
          if(debug){Rprintf("i: %d j: %d i2: %d j2: %d\n\r",i,j,i2,j2);}
          //compute obs for partition:
          obs_cells(0) = compute_obs(ecdf, 0 ,i        ,0  ,i2        );
          obs_cells(1) = compute_obs(ecdf, 0 ,i        ,i2 ,j2        );
          obs_cells(2) = compute_obs(ecdf, 0 ,i        ,j2 ,nr_atoms  );
          ...
          //compute expected for partition:
          exp_cells(0) = compute_expected_uneven_atoms( 0,                   AtomPoint_xaxis(i),
                                                        0,                   AtomPoint_yaxis(i2), n);
          
          exp_cells(1) = compute_expected_uneven_atoms( 0,                   AtomPoint_xaxis(i),
                                                        AtomPoint_yaxis(i2), AtomPoint_yaxis(j2), n);
          
          exp_cells(2) = compute_expected_uneven_atoms( 0,                   AtomPoint_xaxis(i),
                                                        AtomPoint_yaxis(j2), n                  , n);
          
          exp_cells(3) = compute_expected_uneven_atoms( AtomPoint_xaxis(i),  AtomPoint_xaxis(j),
                                                        0,                   AtomPoint_yaxis(i2), n);
          
          exp_cells(4) = compute_expected_uneven_atoms( AtomPoint_xaxis(i),  AtomPoint_xaxis(j),
                                                        AtomPoint_yaxis(i2), AtomPoint_yaxis(j2), n);
          
          exp_cells(5) = compute_expected_uneven_atoms( AtomPoint_xaxis(i),  AtomPoint_xaxis(j),
                                                        AtomPoint_yaxis(j2), n                  , n);
          
          exp_cells(6) = compute_expected_uneven_atoms( AtomPoint_xaxis(j),  n                 ,
                                                        0,                   AtomPoint_yaxis(i2), n);
          
          exp_cells(7) = compute_expected_uneven_atoms( AtomPoint_xaxis(j), n                  ,
                                                        AtomPoint_yaxis(i2) ,AtomPoint_yaxis(j2), n);
          
          exp_cells(8) = compute_expected_uneven_atoms( AtomPoint_xaxis(j), n                   ,
                                                        AtomPoint_yaxis(j2) ,n                  , n);
          ...
          current_max_loglik = 0.0;
          for(int k=0;k<9;k++){
            if(debug){Rprintf("cell %d  - Obs:%f  Exp:%f \n\r",k,obs_cells(k),exp_cells(k));}
            current_max_chisq += (obs_cells(k) - exp_cells(k))*(obs_cells(k) - exp_cells(k))/exp_cells(k);
            if(obs_cells(k)>0.0){
              current_max_loglik += obs_cells(k) * log(obs_cells(k)/exp_cells(k));
            }
          ...
          //check if new max has been attained
          if(current_max_chisq > max_chisq(0)){
            max_chisq(0) = current_max_chisq;
            selected_chisq_i(0) = i;
            selected_chisq_j(0) = j;
            selected_chisq_i2(0) = i2;
